...
- (instancetype)dh_initWithFrame:(CGRect)frame imageNameArray:(NSArray<NSString *> *)imageNameArray buttonIsHidden:(BOOL)isHidden {
    if ([super initWithFrame:frame]) {
        ...
        // 设置引导页上的跳过按钮
        UIButton* skipButton = [HYTool getButtonWithFrame:CGRectMake(kScreen_Width-24-60, 24, 60, 26) title:@"跳过" titleSize:16 titleColor:[UIColor whiteColor] backgroundColor:nil blockForClick:nil];
        [HYTool configViewLayerFrame:skipButton WithColor:[UIColor whiteColor]];
        [HYTool configViewLayerRound:skipButton];
        ...
        for (int i=0; i<imageNameArray.count; i++) {
            ...
            if (i == imageNameArray.count-1 && isHidden == NO) {
                ...
                UIButton* startButton = [HYTool getButtonWithFrame:CGRectMake((kScreen_Width-109)/2, 457*kScale_height, 109, 40) title:@"开始体验" titleSize:18 titleColor:[UIColor whiteColor] backgroundColor:nil blockForClick:nil];
                [HYTool configViewLayerFrame:startButton WithColor:[UIColor whiteColor]];
                [HYTool configViewLayer:startButton size:8];
        ...
        self.imagePageControl.pageIndicatorTintColor = [UIColor colorWithString:@"dcdcdc"];
        self.imagePageControl.currentPageIndicatorTintColor = [UIColor colorWithString:@"484848"];
