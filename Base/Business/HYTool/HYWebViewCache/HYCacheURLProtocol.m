...
@interface HYUrlCacheConfig: NSObject
...
@implementation HYUrlCacheConfig
...
+ (instancetype)instance{
    static HYUrlCacheConfig *urlCacheConfig = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        urlCacheConfig = [[HYUrlCacheConfig alloc] init];
    });
    return urlCacheConfig;
}

- (void)clearUrlDict{
    [HYUrlCacheConfig instance].urlDict = nil;
}
...
+(void)setConfig:(NSURLSessionConfiguration *)config {
    [[HYUrlCacheConfig instance] setConfig:config];
}

+(void)setUpdateInterval:(NSInteger)updateInterval {
    [[HYUrlCacheConfig instance] setUpdateInterval:updateInterval];
}

+(void)clearUrlDic {
    [[HYUrlCacheConfig instance] clearUrlDict];
}
...
- (void)backgroundCheckUpdate{
    ...
    [[[HYUrlCacheConfig instance] backgroundNetQueue] addOperationWithBlock:^{
        NSDate *updateDate = [[HYUrlCacheConfig instance].urlDict objectForKey:weakSelf.request.URL.absoluteString];
        if (updateDate) {
            //判读两次相同的url地址发出请求相隔的时间，如果相隔的时间小于给定的时间，不发出请求。否则发出网络请求
            NSDate *currentDate = [NSDate date];
            NSInteger interval = [currentDate timeIntervalSinceDate:updateDate];
            if (interval < [HYUrlCacheConfig instance].updateInterval) {
                return;
            }
        }
...
- (void)netRequestWithRequest:(NSURLRequest *)request{
    ...
    self.session = [NSURLSession sessionWithConfiguration:config delegate:self delegateQueue:[HYUrlCacheConfig instance].forgeroundNetQueue];
    NSURLSessionDataTask * sessionTask = [self.session dataTaskWithRequest:request];
    [[HYUrlCacheConfig instance].urlDict setValue:[NSDate date] forKey:self.request.URL.absoluteString];
    [sessionTask resume];
}
