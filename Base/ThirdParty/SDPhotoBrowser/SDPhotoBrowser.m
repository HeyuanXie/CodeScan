...
- (void)setupImageOfImageViewForIndex:(NSInteger)index
{
    ...
    if (self.images.count > index) {
        [imageView sd_setImageWithURL:[NSURL URLWithString:self.images[index]] placeholderImage:nil];
    }
...
- (void)photoClick:(UITapGestureRecognizer *)recognizer
{
    ...
    UIView *sourceView = self.sourceImagesContainerView.subviews[currentIndex]; //原小图
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (!_willDisappear) {
        _indexLabel.text = [NSString stringWithFormat:@"%d/%ld", index + 1, (long)self.images.count];
    }
